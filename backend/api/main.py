...
def get_github_trends(
    limit: int = 10, 
    trending_on_hn: bool = Query(False, description="Only show repos explicitly discussed on HN"),
    db: Session = Depends(get_db_session)
):
...
def get_hacker_news_trends(limit: int = 10, db: Session = Depends(get_db_session)):
    ...
    statement = select(HackerNewsStory).order_by(HackerNewsStory.score_growth_6h.desc()).limit(limit)
    return db.exec(statement).all()

@app.get("/api/repositories/{repo_name}/contextual-trends")
def get_repository_contextual_trends(repo_name: str, db: Session = Depends(get_db_session)):
    """
    ...
    fallback_stories = []
    if repo.language:
        hn_statement = select(HackerNewsStory).where(
            HackerNewsStory.title.ilike(f"%{repo.language}%")
        ).order_by(HackerNewsStory.score_growth_6h.desc()).limit(5)
...
def debug_links(db: Session = Depends(get_db_session)):
    ...
    statement = select(HackerNewsStory).where(HackerNewsStory.github_repo_name.is_not(None))
...
def analyze_trends_with_ai(request: ChatRequest, db: Session = Depends(get_db_session)):
    ...
    repos = db.exec(select(GitHubRepo).order_by(GitHubRepo.stars_growth_24h.desc()).limit(10)).all()
    stories = db.exec(select(HackerNewsStory).order_by(HackerNewsStory.score_growth_6h.desc()).limit(10)).all()

    # 2. Fetch direct explicit cross-references
    linked_data = db.exec(
        select(HackerNewsStory, GitHubRepo)
        .join(GitHubRepo, HackerNewsStory.github_repo_name == GitHubRepo.repo_name)
    ).all()
    ...
    for lang in unique_languages:
        ...
        if matching_hn:
            macro_trends_summary.append(f"Language Ecosystem '{lang}' is hot on HN with discussions like: {matching_hn}")

    # 4. Construct layered architectural context
    linked_info = [f"'{l[1].repo_name}' is explicitly mentioned in '{l[0].title}'" for l in linked_data]
